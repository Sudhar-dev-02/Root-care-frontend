...
const PartnerLogin = () => {
  const { partnerLogin } = useContext(AuthContext);
  const navigate = useNavigate();

  const [formData, setFormData] = useState({ email: 'rajesh@caringroots.com', password: 'partner123' });
  ...
  const handleFormSubmit = async (e) => {
    ...
    const res = await partnerLogin(formData.email, formData.password);
    setLoading(false);

    if (res && res.success) {
      navigate('/partner/dashboard');
    } else {
      setErrorMsg(res?.error || 'Invalid partner credentials.');
    }
  };
      ...
      <div className="premium-card w-100 shadow-lg" style={{ maxWidth: '440px', border: 'none', borderRadius: '32px' }}>
        ...
        <div className="p-4 bg-white" style={{ borderBottomLeftRadius: '32px', borderBottomRightRadius: '32px' }}>
          ...
          <form onSubmit={handleFormSubmit}>
            ...
            >
              {loading ? (
                ...
                <span>Access Partner Station</span>
              )}
            </button>
          </form>
          ...
          <div className="mt-4 p-3 rounded text-center small" style={{ backgroundColor: '#F9FAFB', border: '1px solid #F3F4F6' }}>
            <span className="text-muted">Seeded Partner: </span>
            <span className="fw-semibold text-dark">rajesh@caringroots.com</span>
            <span className="text-muted"> / </span>
            <span className="fw-semibold text-dark">partner123</span>
          </div>
